...
import {
    InnerFamilyEdgeData,
    InnerFamilyTypeKey,
} from '../FamilyComponents/InnerFamilyEdge';
import {
    EDGE_YGAP_MODIFIER,
    EDGES_COLORS,
    GENERATION_HEIGHT,
...
import {
    isOddModifer,
    isRelationSharingKids as isRelationPossibleCouple,
} from './utils';
...
function buildEdge({
    ...
    familyIndex,
    offsetY,
}: PreEdge & { offsetY: number }): Edge<InnerFamilyEdgeData> {
...
function findGenerationById(
    ...
    const rawGeneration = Object.entries(generationsToFamilyMembers).find(
        ([, familyMembers]) => familyMembers.find((member) => member.id == id),
    );
...
export function buildEdgesFromParentChildrenRelations(
    ...
    const preEdges = parentChildrenFamilies
        .flatMap((family, i) => {
            const parentsGeneration = findGenerationById(
                generationsToFamilyMembers,
                family.parentA,
            );
    ...
    const edgesWithGap = preEdges.map((currentEdge) => {
        ...
        const thisEdgeIndex = uniqAndSortedEdges.findIndex(
            (edge) => edge == currentEdge.familyIndex,
        );

        const offsetY =
            GENERATION_HEIGHT / 2 +
            (thisEdgeIndex + (thisEdgeIndex % 2)) *
                isOddModifer(thisEdgeIndex) *
                EDGE_YGAP_MODIFIER;
        const limitedOffsetY = Math.max(
            Math.min(offsetY, GENERATION_HEIGHT - 30),
            20,
        );
...
function getChildrenFromCouple(
    parentA: string,
    parentB: string,
    parentChildrenFamilies: ParentsChildrens[],
) {
    return parentChildrenFamilies.find(
        (parentChildren) =>
            (parentChildren.parentA == parentA &&
                parentChildren.parentB == parentB) ||
            (parentChildren.parentB == parentA &&
                parentChildren.parentA == parentB),
    )?.children;
}
...
export function buildCouplesEdges(
    ...
    const dupedCoupleEdges = familyRelationsValues
        ...
        .map((relation) => {
            const children = getChildrenFromCouple(
                relation.from,
                relation.to,
                parentChildrenFamilies,
            );
            const color = children
                ? parentChildEdges.find((edge) =>
            ...
            const edge: Edge = {
                ...
                style: {
                    ...
                    strokeWidth: 3,
                    strokeDasharray:
                        relation.relationType == 'Partner' ? undefined : '5',
                },
